// Cloudflare Pages Function - functions/api/ivyfi-meta.js
// Self-contained - fzstd inlined (MIT license, github.com/101arrowz/fzstd)
...
export async function onRequest(context) {
    ...
    try {
        ...
        // Debug: check fzstd loaded correctly
        const fzstdKeys = Object.keys(fzstd);
        const hasDecompress = typeof fzstd.decompress === 'function';
        
        if (!hasDecompress) {
            return new Response(JSON.stringify({ 
                error: 'fzstd not loaded', 
                keys: fzstdKeys,
                compressedLen: compressed.length,
                magicHex: Array.from(compressed.slice(0,4)).map(b=>b.toString(16).padStart(2,'0')).join('')
            }), { status: 500, headers });
        }
        ...
        return new Response(JSON.stringify({ 
            error: e.message, 
            stack: e.stack,
            fzstdLoaded: typeof fzstd !== 'undefined',
            fzstdDecompress: typeof fzstd?.decompress
        }), { status: 500, headers });
    }
}
